...
class Match:
    ...
    def get_winner(
        self,
    ) -> Tuple[Optional[Contender], Optional[float], Optional[float]]:
        """
        ...
        if self.result.winner == self.contender1.id:
            return (
                self.contender1,
                self.result.contender1_score,
                self.result.contender2_score,
            )
    ...
    def from_model(
        ...
        if not contender1 or not contender2:
            raise ValueError(
                f"Contender not found: {model.contender1_id if not contender1 else model.contender2_id}"
            )
        ...
        if model.result:
            ...
            for name, scores in model.result.criteria_scores.items():
                criteria_scores[name] = CriteriaScoreModel(
                    contender1=scores.contender1, contender2=scores.contender2
                )
