...
def parse_arguments():
    ...
    parser.add_argument(
        "--headless", 
        action="store_true",
        help="Run tournaments in headless mode"
    )
...
def run_consistency_analysis(results_by_model: Dict[str, List[str]]):
    ...
    command = [
        ...
        "--group-by", "config.llm.default_model",
        "--verbose"
    ]
    ...
    for model, files in results.items():
        ...
        if files:
            ...
            with open(first_file, 'r', encoding='utf-8') as f:
                try:
                    data = json.load(f)
                    contenders = len(data.get("statistics", {}).get("total_contenders", 0))
