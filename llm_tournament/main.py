...
def parse_arguments():
    ...
    parser.add_argument(
        "--contenders", 
        type=str, 
        help="Path to contenders JSON file"
    )
...
def run_tournament(args: argparse.Namespace) -> int:
    ...
    prompt_manager = PromptManager(config.get_setting("prompts", "directory"), config.config)
    ...
    try:
        ...
        log_manager.log_info(f"Loaded {len(contenders)} contenders and assessment framework '{framework.id}'")
    except Exception as e:
