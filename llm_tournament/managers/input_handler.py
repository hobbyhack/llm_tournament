...
class InputHandler:
    ...
    def load_contenders(self, file_path: str) -> List[Contender]:
        ...
        try:
            ...
            if "contenders" not in data or not isinstance(data["contenders"], list):
                raise ValueError("Invalid contenders file format. Expected 'contenders' array.")
    ...
    def load_framework(self, file_path: str) -> AssessmentFramework:
        ...
        try:
            ...
            if "assessment_framework" not in data:
                raise ValueError("Invalid framework file format. Expected 'assessment_framework' object.")
            
            framework_data = data["assessment_framework"]
            
            # Validate required fields
            required_fields = ["id", "description", "evaluation_criteria", "comparison_rules", "scoring_system"]
            for field in required_fields:
                if field not in framework_data:
                    raise ValueError(f"Assessment framework missing required field: {field}")
    ...
    def validate_input(self, contenders: List[Contender], framework: AssessmentFramework) -> bool:
        """
    ...
    def load_from_directory(self, directory: str) -> Tuple[Optional[List[Contender]], Optional[AssessmentFramework]]:
        """
