...
class OutputHandler:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        """
        self.config = config
        self.results_file = config.get("output", {}).get("results_file", "./results/tournament_results.json")
        self.match_log_dir = config.get("output", {}).get("match_log_dir", "./results/matches")
        
        # Create directories if they don't exist
        os.makedirs(os.path.dirname(self.results_file), exist_ok=True)
        os.makedirs(self.match_log_dir, exist_ok=True)
    
    def export_tournament_results(self, tournament: Tournament, file_path: Optional[str] = None) -> str:
        """
    ...
    def format_standings(self, standings: List[Dict[str, Any]]) -> str:
        ...
        name_width = max(10, max(len(c["content"]) if len(c["content"]) <= 20 
                               else len(c["content"][:17] + "...") for c in standings))
        stat_width = 8
    ...
    def format_match_result(self, match: Match) -> str:
        ...
        return f"Match {match.id}: {match.contender1.id} vs {match.contender2.id}\n" + \
               f"{winner_str}\n" + \
               f"Scores by criteria: {criteria_str}\n" + \
               f"Rationale: {match.result.rationale[:100]}..." if len(match.result.rationale) > 100 else match.result.rationale